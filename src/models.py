...
def _fit_predict_model(name, Xtr, ytr, Xv, yv, Xte, seed):
    if name == "logistic":
        ...
        model = LogisticRegression(max_iter=1000, class_weight="balanced", random_state=seed, n_jobs=None)
        model.fit(Xtr, ytr)
    ...
    for name in model_names:
        try:
            if name == "persistence":
                ...
                probs = _run_hmm(Xtr, ytr, Xv, Xte, seed)
            else:
                probs = _fit_predict_model(name, Xtr, ytr.astype(int), Xv, yv.astype(int), Xte, seed)
            ...
            for split, meta, prob in zip(["train", "val", "test"], [mtr, mv, mte], probs):
                frames.append(_prediction_frame(meta, prob, threshold, name, "classical", seed, horizon, lookback, experiment, ablation))
...
def _run_hmm(Xtr, ytr, Xv, Xte, seed):
    ...
    return model.predict_proba(Xtr[:, idx])[:, fragile_state], model.predict_proba(Xv[:, idx])[:, fragile_state], model.predict_proba(Xte[:, idx])[:, fragile_state]
